...
public class MusicChangeReceiver extends BroadcastReceiver {
    private static final MusicChangeReceiver instance;
    private final Set<ReceiverListener> listeners = new HashSet<>();

    static {
        instance = new MusicChangeReceiver();
    }
    ...
    public static <Model> void addReceiverListener(ReceiverListener listener) {
        Set<ReceiverListener> listeners = instance.getReceiverListener();
        listeners.add(listener);
    }

    public static <Model> void removeReceiverListener(ReceiverListener listener) {
        Set<ReceiverListener> listeners = instance.getReceiverListener();
        if (listeners == null)
            return;
